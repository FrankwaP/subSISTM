...
simul <- function(individus = ind, 
  ...
  df <- df %>% mutate(x8 = ifelse(df$individus %% 2 == 0, 1, 0))
  
  df$y <- gamma[df$individus,1] + gamma[df$individus,2]*df$x1*df$x5 + gamma[df$individus,3]*df$x2*df$x6
