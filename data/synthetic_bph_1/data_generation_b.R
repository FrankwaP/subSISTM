...
CLSTR_FILE <- "cluster.txt"
SIM_DIR <- "Simulations"
RSLT_DIR <- "Résultats"
# ORCL_DIR <- "Models_oracles"
file.remove(CLSTR_FILE)
dir.create(SIM_DIR, showWarnings = TRUE, recursive = FALSE, mode = "0777")
dir.create(RSLT_DIR, showWarnings = TRUE, recursive = FALSE, mode = "0777")
# dir.create(ORCL_DIR, showWarnings = TRUE, recursive = FALSE, mode = "0777")
...
colnames(dataframe) <- c(SBJCT, TIME)
# Epsilon
...
boucle <- foreach(
  i = 1:100,
  .combine = cbind,
  .packages = c("rockchalk", "dplyr", "lcmm", "doParallel", "foreach")
) %dopar% {
  Dtrain <- simul()
  k <- as.character(i)
  ...
  oracle_mixed <- hlme(y_mixed_obs ~ x2_x5 + x4_x7 + x6_x8,
    ...
    idiag = TRUE,
    data = Dtrain, subject = SBJCT
  )
  # save(oracle_mixed, file = paste0(ORCL_DIR, "/oracle_mix", k, ".rda"))
  ...
  pred_train_mixed <- predictY(oracle_mixed, newdata = Dtrain, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  pred_test_mixed <- predictY(oracle_mixed, newdata = Dtest, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  pred_train_naif_mixed <- predictY(naif_mixed, newdata = Dtrain, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  pred_test_naif_mixed <- predictY(naif_mixed, newdata = Dtest, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  pred_train_lin_mixed <- predictY(lin_mixed, newdata = Dtrain, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  pred_test_lin_mixed <- predictY(lin_mixed, newdata = Dtest, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  # save(oracle_fixed, file = paste0("oracle_fix", k, ".rda"))
  ...
  pred_train_naif_fixed <- predictY(naif_fixed, newdata = Dtrain, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  pred_test_naif_fixed <- predictY(naif_fixed, newdata = Dtest, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  lin_simple_fixed <- lm(y_fixed ~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8, data = Dtrain)

  pred_train_lin_simple_fixed <- predict(lin_simple_fixed, newdata = Dtrain)
  mae_train_lin_simple_fixed_truth <- mean(abs(pred_train_lin_simple_fixed - Dtrain$y_fixed))
  mse_train_lin_simple_fixed_truth <- mean((pred_train_lin_simple_fixed - Dtrain$y_fixed)^2)
  mae_train_lin_simple_fixed_obs <- mean(abs(pred_train_lin_simple_fixed - Dtrain$y_fixed_obs))
  mse_train_lin_simple_fixed_obs <- mean((pred_train_lin_simple_fixed - Dtrain$y_fixed_obs)^2)

  pred_test_lin_simple_fixed <- predict(lin_simple_fixed, newdata = Dtest)
  mae_test_lin_simple_fixed_truth <- mean(abs(pred_test_lin_simple_fixed - Dtest$y_fixed))
  mse_test_lin_simple_fixed_truth <- mean((pred_test_lin_simple_fixed - Dtest$y_fixed)^2)
  mae_test_lin_simple_fixed_obs <- mean(abs(pred_test_lin_simple_fixed - Dtest$y_fixed_obs))
  mse_test_lin_simple_fixed_obs <- mean((pred_test_lin_simple_fixed - Dtest$y_fixed_obs)^2)
  ...
  pred_train_lin_fixed <- predictY(lin_fixed, newdata = Dtrain, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  pred_test_lin_fixed <- predictY(lin_fixed, newdata = Dtest, var.time = TIME, marg = FALSE, subject = SBJCT)
  ...
  res <- c(
    ...
    mae_train_lin_simple_fixed_truth, mse_train_lin_simple_fixed_truth, mae_test_lin_simple_fixed_truth, mse_test_lin_simple_fixed_truth,
    mae_train_lin_simple_fixed_obs, mse_train_lin_simple_fixed_obs, mae_test_lin_simple_fixed_obs, mse_test_lin_simple_fixed_obs
  )
  res <- data.frame(res)
  rownames(res) <- c(
    "mae_train_mixed_truth", "mse_train_mixed_truth", "mae_test_mixed_truth", "mse_test_mixed_truth",
  ...
  Dtrain[, "pred_naif_fixed"] <- pred_train_naif_fixed$pred[, "pred_ss"]
  Dtrain[, "pred_lin_mixed"] <- pred_train_lin_mixed$pred[, "pred_ss"]
  Dtrain[, "pred_lin_fixed"] <- pred_train_lin_fixed$pred[, "pred_ss"]
  Dtrain[, "pred_lin_simple_fixed"] <- pred_train_lin_simple_fixed

  Pred_test_k <- Dtest[, c(SBJCT, TIME)]
  ...
  Pred_test_k[, paste("pred_lin_simple_fixed", k, sep = "_")] <- pred_test_lin_simple_fixed
  Pred_test_k[, paste("pred_naif_fixed", k, sep = "_")] <- pred_test_naif_fixed$pred[, "pred_ss"]

  # sortie
  write.csv2(x = Dtrain, file = paste0(SIM_DIR, "/simulation", k, ".csv"), row.names = FALSE)
...
predictions <- join_all(boucle[2, ], by = c(SBJCT, TIME))
...
write.csv2(x = Dtest, file = paste0(SIM_DIR, "/01_test.csv"), row.names = FALSE)
write.csv(x = predictions, file = paste0(RSLT_DIR, "/Predictions.csv"))
write.csv(x = results, paste0(RSLT_DIR, "/Résultats simulation.csv"))
write.csv(x = truthY, paste0(RSLT_DIR, "/valeurs Y.csv"))
write.csv(x = truthX, paste0(RSLT_DIR, "/valeurs X.csv"))
# write.csv(x = val_moy, file = "valeurs_moyennes.csv")
write.csv(x = scores_moy, file = paste0(RSLT_DIR, "/Performances_moyennes.csv"))
