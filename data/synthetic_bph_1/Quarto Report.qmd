...
for (k in 1:num_simulations) {
  ...
  pred <-predictY(oracle_mixed, Dtest)
  mse_test_oracle[k] <- mean((unlist(pred)- Dtest$mixed)^2)
  mae_test_oracle[k] <- mean(abs(unlist(pred)-Dtest$y_mixed))
  
  #predict_oracle <- predict(oracle_mixed, newdata = Dtest)
  #mse_test_oracle[k] <- mean((Dtest$y_mixed_obs - predict_oracle)^2)
...
mse_train_oracle <- data.frame(mse_train_oracle)
?hlme
mae_train_oracle <- data.frame(mae_train_oracle)
```
