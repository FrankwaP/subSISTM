...
def _get_2D_unscaled_prediction_list(
    model_list: list[ModelConfiguration],
    test_data: Data,
    n_cpus: int,
) -> list[DataFrame]:
    return [
        test_data.to_2D_unscaled(ypred_3D_scaled=pred_3D)
        for pred_3D in get_3D_scaled_prediction_list(
            model_list, test_data, n_cpus
        )
    ]
...
def get_predictions_dataframe(
    ...
    y_labels = test_data.y_labels
    assert len(y_labels) == 1
    y_label = y_labels[0]
    sercol = test_data.series_column_name
    tstepcol = test_data.timestep_column_name
    data_test = test_data.dataframe[[sercol, tstepcol, y_label]]
    ...
    data_test.loc[:, "mean-absolute-error"] = (
        data_test[err_cols].abs().mean(axis=1)
    )
...
def remove_warmups(
    ...
    return pred_dataframe[
        pred_dataframe[timestep_column_name] > times[N_warmups]
    ]
...
def plot_individual_results(
    ...
    indiv_dataframe = pred_dataframe[
        pred_dataframe[serie_column_name] == ind_idx
    ]
