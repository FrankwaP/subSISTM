...
fct_fit_esn <- function(X,
 ...
 if (link_source) {
  source <- reservoirnet::createNode(nodeType = "Input")
  ...
  if(importance){
    ...
    if(link_source){
      ...
      size_concatenator = model$nodes[3]      
      if(size_concatenator[[1]]$input_dim[[1]] == ncol(X) & size_concatenator[[1]]$input_dim[[2]] == as.numeric(units)){
        dfimp <- dfimp %>%
          mutate(feature = c(colnames(X), paste0("reservoir", 1:as.numeric(units))), .before = 1)
      } else if(size_concatenator[[1]]$input_dim[[1]] == as.numeric(units) & size_concatenator[[1]]$input_dim[[2]] == ncol(X)){
        dfimp <- dfimp %>%
