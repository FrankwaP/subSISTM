...
lsdataDatesList <- fct_load_data_find_hp()
##### META PARAMETERS
...
df_fct_hp <- list(ridge = function(n) reservoirnet::rloguniform(n = n, min = 1e-5, max = 1e10),
...
# get all features
dataset_temp <- lsdataDatesList[[1]]
vec_features <- fct_smoothing_derivative(dataset_temp$data_covid,
                                         maxOutcomeDate = max(dataset_temp$data_covid$START_DATE)) %>%
...
dfres <- lapply(seq_len(n_samples),
                function(id_sample){
                  ...
                  time_start <- Sys.time()
                  mean_absolute_error <- fct_objective(lsdataDatesList,
                                                       warmup = 30,
                                                       forecast_days = 14,
                                                       units = 500,
