...
fct_iterative_forecast_from_hp <- function(datasets,
                                           link_source = FALSE,
                                           model = "esn",
                                           warmup = 30,
                                           forecast_days = 14,
  ...
  lapply(names(datasets), function(dataset_name){
    data_covid <- datasets[[dataset_name]]$data_covid
    lapply(datasets[[dataset_name]]$vecDates, function(date_i){
      ...
      if(model == "esn"){
        seed <- ifelse(length(names(datasets)) > 1,
                        yes = round(runif(n = 1,
                                        min = 1,
                                        max = 1e5)),
                        no = seed) 
        forecast_deriv <- fct_fit_esn(
          ...
          importance = importance
                      ...
                      forecast_deriv <- fct_fit_enet(X = ls_train_test$X,
                                                     ...
                                                     alpha = alpha[1],
                                                     importance = importance)
        print(forecast_deriv)
      } else {
        stop("unknown model argument")
      }
      if (importance) {
        dfPredAll <<- fct_aggregate_forecast(data_i = data_i, forecast_deriv = forecast_deriv$pred) %>%
          mutate(iter = 1, outcomeDate = date_i) %>%
          bind_rows(dfPredAll)
        ...
        dfPredAll <<- fct_aggregate_forecast(data_i = data_i, forecast_deriv = forecast_deriv) %>%
          mutate(iter = 1, outcomeDate = date_i) %>%
          bind_rows(dfPredAll)
      }
