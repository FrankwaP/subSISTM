...
##### LOAD DATA
data_files <- list.files(path = "data/mice_scaled/", pattern = "*.rds", full.names = TRUE)
##### COMMON PARAMETERS
forecast_days = 14
warmup = 30
units = 500
...
importance = TRUE
##### LOAD BEST HP SETS
hp_sets = list(common_input_scaling = "data/common_input_scaling_linked_13867155/",
               common_input_scaling_linked_source = "data/common_input_scaling_13873713/",
               multiple_input_scaling = "data/multiple_input_scaling_linked_13867155/",
               multiple_input_scaling_linked_source = "data/multiple_input_scaling_13873713/"
               ) %>%
...
datasets[[basename(data_file)]] <- list(data_covid = data_covid, vecDates = vecDates[slar_taskid])
}

message(paste0("--------- evaluation date : ", " ----------"))
...
lapply(names(best_hp_set),
       function(model_set){
         ...
         if(model_set == "enet"){
           ##### ELASTIC-NET
           forecast_enet_once <- fct_iterative_forecast_from_hp(datasets,
         ...
         if(model_set %in% c("common_input_scaling",
           ...
           lapply(X = seq_len(nrow(hp_set)),
                  function(row_i){
                    ...
                    forecast_esn <- fct_iterative_forecast_from_hp(datasets,
                                                                   warmup = warmup,
                                                                   forecast_days = forecast_days,
                                                                   units = units,
